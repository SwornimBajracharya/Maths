...
public class VectorP extends JInternalFrame {
	private JPanel panel;
	private JPanel panel_1;
	private JLabel lblAddition;
	private JTextField dotField;
	...
	private JPanel getPanel_1() {
		if (panel_1 == null) {
			...
			panel_1.add(getDotField());
			panel_1.add(getLblSubtraction());
			panel_1.add(getCrossField());
		}
		return panel_1;
	}
	private JLabel getLblAddition() {
		if (lblAddition == null) {
			lblAddition = new JLabel("Dot Product");
			lblAddition.setFont(new Font("SansSerif", Font.PLAIN, 18));
			lblAddition.setBounds(12, 15, 142, 24);
		}
		return lblAddition;
	}
	private JTextField getDotField() {
		if (dotField == null) {
			dotField = new JTextField();
			dotField.setEnabled(false);
			dotField.setFont(new Font("SansSerif", Font.PLAIN, 18));
			dotField.setBounds(166, 11, 122, 28);
			dotField.setColumns(10);
		}
		return dotField;
	}
	private JLabel getLblSubtraction() {
		if (lblSubtraction == null) {
			lblSubtraction = new JLabel("Cross Product: ");
			lblSubtraction.setFont(new Font("SansSerif", Font.PLAIN, 18));
			lblSubtraction.setBounds(12, 53, 142, 24);
		}
		return lblSubtraction;
	}
	private JTextField getCrossField() {
		if (crossField == null) {
			crossField = new JTextField();
			crossField.setEnabled(false);
			crossField.setFont(new Font("SansSerif", Font.PLAIN, 18));
			crossField.setBounds(166, 49, 122, 28);
		...
		if (btnCalculate == null) {
			...
			btnCalculate.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					dotField.setText(String.valueOf((a1 * b1) + (a2 * b2) + (a3 * b3)));
					crossField.setText((a2*b3 - b2*a3) + ", " + (a1*b3 - b1*a3) + ", " + (a1*b2 - b1*a2));
				}
			});
